...
import './App.css';
import Color from './contracts/MinerNFT.json';
...
function hashToColor(str){
	if (str.length < 7) {
		throw new Error("Hash is not long enough to get a color hex code");
	}
	const hexString = '0x' + str.substring(2, 10);
...
class App extends Component {
	...
	newMiner = (hashMAC) => {
		const colorBytes = hashToColor(hashMAC);
		this.state.contract.methods
			.mint(colorBytes)
			.send({
				from: this.state.account
			...
			.once('receipt', (receipt) => {
				console.log('transaction receipt: ', receipt)
				this.setState({
					colors: [...this.state.colors, hashMAC],
				});
			});
	}
